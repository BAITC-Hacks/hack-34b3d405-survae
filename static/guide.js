...
(function(){
 ...
 async function startKitchen(container){
  ...
  await withBusy(async()=>{
   await api('/api/chat',{action:'reset'});
 ...
 function step(title,description,choices,number=1){
  const session=version,container=help('');container.append(el('div','guide-progress',`ПОДБОР ЛАМПЫ · ШАГ ${number} ИЗ 4`),el('h2','guide-title',title),el('p','guide-description',description));
 ...
 function start(){if(busy)return;if(!csrf){toast('Дождитесь подключения к серверу.');return;}version++;journey={};const n=help('Названия знать не обязательно. Выберите, что хотите сделать, или напишите задачу своими словами.');const row=el('div','guide-start-options');
  if(state?.ai_enabled)row.append(action('Обсудить задачу с AI',()=>{finishStep(n);withBusy(async()=>{await api('/api/chat',{action:'reset'});addUser('Помоги выбрать товары для моей задачи');addResponse(await api('/api/chat',{message:'Помоги выбрать товары. Я не знаю, что мне нужно. Спроси меня о моей задаче.'}));})}));
  row.append(action('✦ Собрать подсветку кухни',()=>startKitchen(n)));
row.append(action('💡 Заменить лампочку',()=>{finishStep(n);withBusy(async()=>{await api('/api/chat',{action:'reset'});journey={};addUser('Хочу заменить лампочку');baseStep()})}),action('⌕ Найти по маркировке или фото',()=>{finishStep(n);identify()}),action('▤ Собрать товары по списку',()=>{finishStep(n);const a=help('Вставьте список с количеством в поле сообщения или прикрепите спецификацию. Добавляйте найденные позиции в «Ваш список», затем подтвердите весь набор одним действием.');a.append(action('Прикрепить спецификацию',()=>$('file-input').click()));$('message-input').focus()}),action('✎ У меня другая задача',()=>{finishStep(n);const a=help('Опишите, что должно получиться и что уже есть. Например: «Нужен свет для рабочего стола, светильник уже есть». Можно приложить маркировку или документ.');a.append(action('Написать задачу',()=>{$('message-input').focus()}));$('message-input').focus()}));n.append(row);scrollBottom();
 }
 ...
 function stage(id,amount){if(busy)return;const p=products.get(String(id));if(!p||p.city!==state.city){toast('Обновите поиск для текущего города.');return}const current=selection.get(String(id))?.quantity||0,already=state.cart.items.find(x=>String(x.id)===String(id))?.cart_quantity||0;const next=current+amount;
  ...
  selection.set(String(id),{product:p,quantity:next});renderSelection();toast('Добавлено в ваш список. Корзина пока не изменилась.');}
 ...
 async function prepareSelection(){if(busy||!selection.size)return;$('selection-dialog').close();await withBusy(async()=>{const data=await api('/api/cart/propose',{items:[...selection.values()].map(x=>({id:x.product.id,quantity:x.quantity}))});currentProposal=data.proposal?.id;addResponse(data)})}
 function afterResponse(data,container){
  if(data.kit){
   const kit=el('section','kit-summary');
   kit.append(el('h3','',data.kit.complete?'Комплект для ваших условий':'Уточняем ваш комплект'));
 ...
 window.FrontendGuide={remember,afterResponse,stage,reset,lock,intercept,ready(serverState){$('build-version').textContent='Версия '+(serverState.version||'local')},confirmed(id){if(id===currentProposal)currentProposal=null},cityChanged(){version++;journey=null;products.clear();selection.clear();renderSelection();toast('Город изменён. Найдите товары заново: наличие зависит от склада.')}};
})();
