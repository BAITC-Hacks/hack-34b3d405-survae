...
async function api(path,body,method='POST'){
 ...
 const result=await fetch(path,options);const data=await result.json();
 if(!result.ok)throw new Error(typeof data.detail==='string'?data.detail:'Не удалось выполнить запрос. Проверьте данные.');
 return data;
}
...
function revealChat(){$('welcome').hidden=true}
...
function addResponse(data){
 ...
 $('messages').append(el);if(data.cart)updateCart(data.cart);if(data.city)$('city').value=data.city;scrollBottom();
}
function addError(error){revealChat();const e=document.createElement('div');e.className='message error-message';e.textContent=error.message||String(error);$('messages').append(e);scrollBottom()}
...
function openCart(){ $('drawer-backdrop').hidden=false;$('cart-drawer').hidden=false;$('close-cart').focus()}
function closeCart(){ $('drawer-backdrop').hidden=true;$('cart-drawer').hidden=true;$('open-cart').focus()}
async function withBusy(fn){if(busy)return;busy=true;$('send-button').disabled=true;const thinking=document.createElement('div');thinking.className='thinking';thinking.innerHTML='<span>Проверяю данные каталога…</span>';$('messages').append(thinking);revealChat();scrollBottom();try{await fn()}catch(e){addError(e)}finally{thinking.remove();busy=false;$('send-button').disabled=false;scrollBottom()}}
async function send(message){message=message.trim();if(!message||busy)return;const current=attachment;addUser(message+(current?`\n📎 ${current.name}`:''));$('message-input').value='';$('message-input').style.height='auto';attachment=null;renderAttachment();await withBusy(async()=>addResponse(await api('/api/chat',{message,attachment_id:current?.attachment_id||null})));$('message-input').focus()}
...
$('new-chat').onclick=async()=>{if(busy)return;try{await api('/api/chat/reset',{});$('messages').replaceChildren();attachment=null;renderAttachment();$('welcome').hidden=false;$('message-input').focus()}catch(e){toast(e.message)}};
document.addEventListener('click',e=>{
 ...
 const add=e.target.closest('[data-add]');if(add){const q=Number(add.parentElement.querySelector('.qty').value);withBusy(async()=>addResponse(await api('/api/cart/propose',{items:[{id:add.dataset.add,quantity:q}]})));return}
 const confirm=e.target.closest('[data-confirm]');if(confirm){withBusy(async()=>addResponse(await api('/api/cart/confirm',{action_id:confirm.dataset.confirm,confirmed:true})));return}
 if(e.target.closest('[data-cancel]')){withBusy(async()=>addResponse(await api('/api/cart/cancel',{})));return}
 const remove=e.target.closest('[data-remove]');if(remove){closeCart();withBusy(async()=>addResponse(await api('/api/cart/propose',{operation:'remove',items:[{id:remove.dataset.remove,quantity:1}]})));return}
...
$('clear-cart').onclick=()=>{closeCart();withBusy(async()=>addResponse(await api('/api/cart/propose',{operation:'clear',items:[]})))};
$('city').onchange=async()=>{try{const r=await api('/api/city',{city:$('city').value});state.city=r.city;updateCart(r.cart);document.querySelectorAll('.proposal button').forEach(b=>b.disabled=true);toast(r.message)}catch(e){$('city').value=state.city;toast(e.message)}};
...
async function loadState(){try{state=await api('/api/state',undefined,'GET');csrf=state.csrf;$('city').value=state.city;updateCart(state.cart);$('catalog-status').textContent=state.catalog.loading?'Загружаем каталог…':state.catalog.mode==='demo'?'Учебный каталог · '+state.catalog.count+' товаров':'Каталог EKT · '+state.catalog.count+' товаров в выборке';$('ai-status').textContent=state.ai_enabled?'AI подключён':'Базовый поиск';$('status-dot').classList.toggle('warn',!!state.catalog.error||state.catalog.mode==='demo');$('setup-link').hidden=!state.setup_allowed||state.ai_enabled;$('stock-example').textContent=state.catalog.mode==='demo'?'Например, DEMO-C16-A':'Например, 200300285_';if(state.catalog.error)toast(state.catalog.error);if(state.catalog.loading)setTimeout(loadState,2500)}catch(e){toast('Нет связи с приложением. Обновите страницу.')}}
